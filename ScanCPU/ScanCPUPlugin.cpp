...
#define kPluginName "ScanCPU"
#define kPluginGrouping "OpenFX Yo"
#define kPluginDescription \
"------------------------------------------------------------------------------------------------------------------ \n" \
"White ScanCPU and Image Analysis. Use ColorPicker or Rectangle to sample pixel values. \n" \
"Derive Min, Max, Mean RGB and HSV values, Median RGB values, and Min, Max Luma values \n" \
"and their pixel coordinates. Apply White ScanCPU with ColorPicker, Mean, or Median source \n" \
...
#define kPluginIdentifier "OpenFX.Yo.ScanCPU"
...
#define kParamScanCPUType "Balance Type"
#define kParamScanCPUTypeLabel "Balance Type"
#define kParamScanCPUTypeHint "White Balance Formula"
#define kParamScanCPUTypeOptionGain "Gain"
#define kParamScanCPUTypeOptionGainHint "Use Gain Formula"
#define kParamScanCPUTypeOptionOffset "Offset"
#define kParamScanCPUTypeOptionOffsetHint "Use Offset Formula"
#define kParamScanCPUTypeOptionLift "Lift"
#define kParamScanCPUTypeOptionLiftHint "Use Lift Formula"

enum ScanCPUTypeEnum
{
    eScanCPUTypeGain,
    eScanCPUTypeOffset,
    eScanCPUTypeLift,
...
public:
    ScanCPUProcessorBase(ImageEffect &instance)
...
public:
    ...
    float bLift, float lumaMath, float lumaLimit, float GainScanCPU, float OffsetScanCPU, 
    float WhiteScanCPU, float PreserveLuma, float DisplayAlpha, float LumaRec709, 
...
private:
    ...
    float _lumaMath[1];
    float _lumaLimit[1];
    float _GainScanCPU[1];
    float _OffsetScanCPU[1];
    float _WhiteScanCPU[1];
...
void ImageScaler::multiThreadProcessImages(OfxRectI p_ProcWindow)
{
    for (int y = p_ProcWindow.y1; y < p_ProcWindow.y2; ++y)
    {
        ...
        for (int x = p_ProcWindow.x1; x < p_ProcWindow.x2; ++x)
        {
			...
			float BalR = _GainScanCPU[0] == 1.0f ? srcPix[0] * _balGain[0] : _OffsetScanCPU[0] == 1.0f ? srcPix[0] + _balOffset[0] : srcPix[0] + (_balLift[0] * (1.0f - srcPix[0]));
			float BalB = _GainScanCPU[0] == 1.0f ? srcPix[2] * _balGain[1] : _OffsetScanCPU[0] == 1.0f ? srcPix[2] + _balOffset[1] : srcPix[2] + (_balLift[1] * (1.0f - srcPix[2]));
			float Red = _WhiteScanCPU[0] ? ( _PreserveLuma[0] ? BalR * _lumaMath[0] : BalR) : srcPix[0];
			float Green = _PreserveLuma[0] ? srcPix[1] * _lumaMath[0] : srcPix[1]; 
			float Blue = _WhiteScanCPU[0] ? ( _PreserveLuma[0] ? BalB * _lumaMath[0] : BalB) : srcPix[2];
...
void ImageScaler::setScales(float rGain, float bGain, float rOffset, float bOffset, float rLift, 
    float bLift, float lumaMath, float lumaLimit, float GainScanCPU, float OffsetScanCPU,
    float WhiteScanCPU, float PreserveLuma, float DisplayAlpha, float LumaRec709, float LumaRec2020, 
	float LumaDCIP3, float LumaACESAP0, float LumaACESAP1, float LumaAvg)
{
    ...
    _GainScanCPU[0] = GainScanCPU;
    _OffsetScanCPU[0] = OffsetScanCPU;
    _WhiteScanCPU[0] = WhiteScanCPU;
...
public:
    /** @brief ctor */
    ScanCPUPlugin(OfxImageEffectHandle handle)
        : ImageEffect(handle)
        , _dstClip(0)
        , _srcClip(0)
        , _btmLeft(0)
        ...
        , _restrictToRectangle(0)
    {
        ...
        m_ScanCPU = fetchRGBParam("balance");
    	m_Rgb = fetchDouble3DParam("rgbVal");
    	m_Hsl = fetchDouble3DParam("hslVal");
    	m_White = fetchBooleanParam("whiteScanCPU");
    	_sampleType = fetchChoiceParam(kParamSampleType);
    	_balanceType = fetchChoiceParam(kParamScanCPUType);
        ...
        bool WhiteScanCPU = m_White->getValue();
        _sampleType->setIsSecretAndDisabled(!WhiteScanCPU);
        _balanceType->setIsSecretAndDisabled(!WhiteScanCPU);
        _preserveLuma->setIsSecretAndDisabled(!WhiteScanCPU);
...
private:
    ...
    void YoProcess(ScanCPUProcessorBase &processor, const Image* srcImg, double time, const OfxRectI &analysisWindow, 
...
void
ScanCPUPlugin::setupAndProcess(ImageScaler& p_ImageScaler, const RenderArguments &args)
{
    ...
    ScanCPUTypeEnum balanceType = (ScanCPUTypeEnum)balanceType_i;
    ...
    RGBdub colorSample;
    m_ScanCPU->getValueAtTime(args.time, colorSample.r, colorSample.g, colorSample.b);
    ...
    float ScanCPUR = ColorSample ? colorSample.r : MeanSample ? meanSample.r : medianSample.r;
    float ScanCPUG = ColorSample ? colorSample.g : MeanSample ? meanSample.g : medianSample.g;
    float ScanCPUB = ColorSample ? colorSample.b : MeanSample ? meanSample.b : medianSample.b;
    
    float rGain = ScanCPUG/ScanCPUR;
    float bGain = ScanCPUG/ScanCPUB;
    
    float rOffset = ScanCPUG - ScanCPUR;
    float bOffset = ScanCPUG - ScanCPUB;
    
    float lumaRec709 = ScanCPUR * 0.2126f + ScanCPUG * 0.7152f + ScanCPUB * 0.0722f;
    float lumaRec2020 = ScanCPUR * 0.2627f + ScanCPUG * 0.6780f + ScanCPUB * 0.0593f;
    float lumaDCIP3 = ScanCPUR * 0.209492f + ScanCPUG * 0.721595f + ScanCPUB * 0.0689131f;
    ...
    //ScanCPUR + (rLift * (1.0 - ScanCPUR)) = ScanCPUG
    float rLift = (ScanCPUG - ScanCPUR) / (1.0f - ScanCPUR);
    float bLift = (ScanCPUG - ScanCPUB) / (1.0f - ScanCPUB);
    ...
    bool whiteScanCPU = m_White->getValueAtTime(args.time);
    float WhiteScanCPU = whiteScanCPU ? 1.0f : 0.0f;
    float PreserveLuma = preserveLuma ? 1.0f : 0.0f;
    float LumaLimit = lumaLimit ? 1.0f : 0.0f;
    float DisplayAlpha = displayAlpha ? 1.0f : 0.0f;
	
    bool gainScanCPU = balanceType_i == 0;
    bool offsetScanCPU = balanceType_i == 1;
    bool liftScanCPU = balanceType_i == 2;
    
    float GainScanCPU = gainScanCPU ? 1.0f : 0.0f;
    float OffsetScanCPU = offsetScanCPU ? 1.0f : 0.0f;
    float LiftScanCPU = liftScanCPU ? 1.0f : 0.0f;
    ...
    p_ImageScaler.setScales(rGain, bGain, rOffset, bOffset, rLift, bLift, lumaMath, lumaLimit, GainScanCPU, OffsetScanCPU, 
    WhiteScanCPU, PreserveLuma, DisplayAlpha, LumaRec709, LumaRec2020, LumaDCIP3, LumaACESAP0, LumaACESAP1, LumaAvg);

    p_ImageScaler.process();
} 

void
ScanCPUPlugin::render(const RenderArguments &args)
{
...
} // ScanCPUPlugin::render

// override the roi call
// Required if the plugin requires a region from the inputs which is different from the rendered region of the output.
// (this is the case here)
void
ScanCPUPlugin::getRegionsOfInterest(const RegionsOfInterestArguments &args,
...
bool
ScanCPUPlugin::getRegionOfDefinition(const RegionOfDefinitionArguments &args,
                                             OfxRectD & /*rod*/)
{
    if ( !kSupportsRenderScale && ( (args.renderScale.x != 1.) || (args.renderScale.y != 1.) ) ) {
        throwSuiteStatusException(kOfxStatFailed);
...
bool
ScanCPUPlugin::isIdentity(const IsIdentityArguments &args,
                                  Clip * &identityClip,
                                  double & identityTime
                                  , int& /*view*/, std::string& /*plane*/)
{
    ...
    float rScanCPU = colorSample.r;
    float gScanCPU = colorSample.g;
    float bScanCPU = colorSample.b;
    

    if ((rScanCPU == 1.0) && (gScanCPU == 1.0) && (bScanCPU == 1.0))
    {
...
void
ScanCPUPlugin::changedParam(const InstanceChangedArgs &args,
                                    const std::string &paramName)
{
    ...
    if (paramName == "whiteScanCPU") {
        // update visibility
        bool WhiteScanCPU = m_White->getValueAtTime(time);
        _sampleType->setIsSecretAndDisabled(!WhiteScanCPU);
        _balanceType->setIsSecretAndDisabled(!WhiteScanCPU);
        _preserveLuma->setIsSecretAndDisabled(!WhiteScanCPU);
    }
...
#     endif
    ...
    bool whiteScanCPU = m_White->getValueAtTime(args.time);
    int WhiteScanCPU = whiteScanCPU ? 1 : 0;
    ...
    int GainScanCPU = gainScanCPU ? 1 : 0;
    int OffsetScanCPU = offsetScanCPU ? 1 : 0;
    int LiftScanCPU = liftScanCPU ? 1 : 0;
	...
	fprintf (pFile, "// ScanCPUPlugin DCTL export\n" \
	"\n" \
	...
	"	float Alpha = alpha > 1.0f ? 1.0f : alpha < 0.0f ? 0.0f : alpha;\n" \
	"\n" \
	"	float BalR = GainScanCPU == 1 ? p_R * balGainR : OffsetScanCPU == 1 ? p_R + balOffsetR : p_R + (balLiftR * (1.0f - p_R));\n" \
	"	float BalB = GainScanCPU == 1 ? p_B * balGainB : OffsetScanCPU == 1 ? p_B + balOffsetB : p_B + (balLiftB * (1.0f - p_B));\n" \
	"	float Red = WhiteScanCPU == 1 ? ( PreserveLuma == 1 ? BalR * lumaMath : BalR) : p_R;\n" \
	"	float Green = WhiteScanCPU == 1 && PreserveLuma == 1 ? p_G * lumaMath : p_G;\n" \
	"	float Blue = WhiteScanCPU == 1 ? ( PreserveLuma == 1 ? BalB * lumaMath : BalB) : p_B;\n" \
	"\n" \
	"	float r = Red * Alpha + p_R * (1.0f - Alpha);\n" \
	"	float g = Green * Alpha + p_G * (1.0f - Alpha);\n" \
	"	float b = Blue * Alpha + p_B * (1.0f - Alpha);\n" \
	"	return make_float3(r, g, b);\n" \
	"}\n", rGain, bGain, rOffset, bOffset, rLift, bLift, lumaMath, lumaLimit, GainScanCPU, OffsetScanCPU, WhiteScanCPU, PreserveLuma, 
...
//} // ScanCPUPlugin::changedParam

/* set up and run a processor */
void
ScanCPUPlugin::YoProcess(ScanCPUProcessorBase &processor,
                                       const Image* srcImg,
                                       double time,
                                       const OfxRectI &analysisWindow,
                                       const Results &prevResults,
...
bool
ScanCPUPlugin::computeWindow(const Image* srcImg,
                                     double time,
                                     OfxRectI *analysisWindow)
{
    OfxRectD regionOfInterest;
...
// update image statistics
void
ScanCPUPlugin::update(const Image* srcImg,
                              double time,
                              const OfxRectI &analysisWindow)
{
...
void
ScanCPUPlugin::updateMed(const Image* srcImg,
                              double time,
                              const OfxRectI &analysisWindow)
{
    // TODO: CHECK if checkDoubleAnalysis param is true and analysisWindow is the same as btmLeft/sizeAnalysis
...
void
ScanCPUPlugin::updateHSV(const Image* srcImg,
                                  double time,
                                  const OfxRectI &analysisWindow)
{
    Results results;
...
void
ScanCPUPlugin::updateLuma(const Image* srcImg,
                                  double time,
                                  const OfxRectI &analysisWindow)
{
    Results results;
...
class ScanCPUInteract
    : public RectangleInteract
{
public:

    ScanCPUInteract(OfxInteractHandle handle,
                            ImageEffect* effect)
...
class ScanCPUOverlayDescriptor
    : public DefaultEffectOverlayDescriptor<ScanCPUOverlayDescriptor, ScanCPUInteract>
{
};

ScanCPUPluginFactory::ScanCPUPluginFactory()
    : OFX::PluginFactoryHelper<ScanCPUPluginFactory>(kPluginIdentifier, kPluginVersionMajor, kPluginVersionMinor)
{
}


void
ScanCPUPluginFactory::describe(ImageEffectDescriptor &desc)
{
    // basic labels
    ...
    desc.setOverlayInteractDescriptor(new ScanCPUOverlayDescriptor);

}


void
ScanCPUPluginFactory::describeInContext(ImageEffectDescriptor &desc,
                                                ContextEnum /*context*/)
{
    ...
    
{
    ...
    BooleanParamDescriptor* boolParam = desc.defineBooleanParam("whiteScanCPU");
	...
	{
            ChoiceParamDescriptor* param = desc.defineChoiceParam(kParamScanCPUType);
            param->setLabel(kParamScanCPUTypeLabel);
            param->setHint(kParamScanCPUTypeHint);
            assert(param->getNOptions() == eScanCPUTypeGain);
            param->appendOption(kParamScanCPUTypeOptionGain, kParamScanCPUTypeOptionGainHint);
            assert(param->getNOptions() == eScanCPUTypeOffset);
            param->appendOption(kParamScanCPUTypeOptionOffset, kParamScanCPUTypeOptionOffsetHint);
            assert(param->getNOptions() == eScanCPUTypeLift);
            param->appendOption(kParamScanCPUTypeOptionLift, kParamScanCPUTypeOptionLiftHint);
            //param->setIsSecretAndDisabled(true);
            page->addChild(*param);
        }
    {  
...
ImageEffect* ScanCPUPluginFactory::createInstance(OfxImageEffectHandle handle, ContextEnum /*context*/)
{
    return new ScanCPUPlugin(handle);
}

void OFX::Plugin::getPluginIDs(PluginFactoryArray& p_FactoryArray)
{
    static ScanCPUPluginFactory scanCPUPlugin;
    p_FactoryArray.push_back(&scanCPUPlugin);
}
