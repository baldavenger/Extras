...
"#define  BLOCKSIZE 4 \n" \
"__kernel void BalanceAdjustKernel(	\n" \
"   int p_Width,	\n" \
"   int p_Height,	\n" \
"   float balGainR,  \n" \
"   float balGainB,  \n" \
...
"      barrier(CLK_LOCAL_MEM_FENCE | CLK_GLOBAL_MEM_FENCE);   \n" \
"                                                                       \n" \
"      float lumaRec = SRC[0] * 0.2126f + SRC[1] * 0.7152f + SRC[2] * 0.0722f;\n" \
"      float lumaAvg = (SRC[0] + SRC[1] + SRC[2]) / 3.0f;\n" \
"      float lumaMax = fmax(SRC[2], fmax(SRC[0], SRC[1]));\n" \
"      float luma = w_LumaRec == 1.0f ? lumaRec : w_LumaAvg == 1.0f ? lumaAvg : lumaMax;\n" \
"      \n" \
...
float* DisplayAlpha, float* LumaRec, float* LumaAvg, const float* p_Input, float* p_Output)
{	
	cl_int error;
...
//#endif
	...
	error |= clSetKernelArg(kernel, count++, sizeof(float), &LumaRec[0]);
