__global__ void BalanceAdjustKernel(int p_Width, int p_Height, float balGainR, float balGainB, float balOffsetR, float balOffsetB,
...
float* LumaACESAP0, float* LumaACESAP1, float* LumaAvg, const float* p_Input, float* p_Output)
{
    ...
    BalanceAdjustKernel<<<blocks, threads>>>(p_Width, p_Height, balGain[0], balGain[1], balOffset[0], balOffset[1],
