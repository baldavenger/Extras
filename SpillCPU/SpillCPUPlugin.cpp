...
#define kPluginName "SpillCPU"
#define kPluginGrouping "OpenFX Yo"
#define kPluginDescription \
"------------------------------------------------------------------------------------------------------------------ \n" \
"Keyer and Despill\n" \

#define kPluginIdentifier "OpenFX.Yo.SpillCPU"
...
class SpillCPUProcessorBase : public OFX::ImageProcessor
{
protected:
...
public:
    ...
    SpillCPUProcessorBase(OFX::ImageEffect &instance)
  ...
  void setSrcImgs(const OFX::Image *srcImg, const OFX::Image *coreImg, const OFX::Image *garbageImg)
    {
        _srcImg = srcImg;
        _coreImg = coreImg;
...
class SpillCPUProcessor : public SpillCPUProcessorBase
{
public:
    SpillCPUProcessor(OFX::ImageEffect &instance)
    : SpillCPUProcessorBase(instance)
    {
    }
...
class SpillCPUPlugin : public OFX::ImageEffect
{
public:
    /** @brief ctor */
    SpillCPUPlugin(OfxImageEffectHandle handle)
    : ImageEffect(handle)
    , _dstClip(0)
    , _srcClip(0)
    , _coreClip(0)
	...
	_despillCore = fetchBooleanParam(kParamDespillCore);
       _outputMode = fetchChoiceParam(kParamOutputMode);
        ...
        assert(_keyColour && _acceptanceAngle && _suppressionAngle && _keyBalance && _keyAmount && _midpoint && 
        _shadows && _midtones && _highlights && _replacementColour && _matteBalance && _despillBalance && 
        _replacementAmount && _preserveLuminance && _despillCore  && _outputMode && _sourceAlpha);
    }
...
private:
    ...
    void setupAndProcess(SpillCPUProcessorBase &, const OFX::RenderArguments &args);
...
/* set up and run a processor */
void
SpillCPUPlugin::setupAndProcess(SpillCPUProcessorBase &processor, const OFX::RenderArguments &args)
{
    ...
    processor.setSrcImgs(src.get(), core.get(), garbage.get());
...
void
SpillCPUPlugin::render(const OFX::RenderArguments &args)
{
    ...
    switch (dstBitDepth) {
            ...
            //    SpillCPUProcessor<unsigned char, 4, 255> fred(*this);
            //    setupAndProcess(fred, args);
            //    break;
            //}
        case OFX::eBitDepthUShort: {
            SpillCPUProcessor<unsigned short, 4, 65535> fred(*this);
            setupAndProcess(fred, args);
            break;
        }
        case OFX::eBitDepthFloat: {
            SpillCPUProcessor<float, 4, 1> fred(*this);
            setupAndProcess(fred, args);
            break;
        }
        default :
...
void
SpillCPUPlugin::getClipPreferences(OFX::ClipPreferencesSetter &clipPreferences)
{
...
//mDeclarePluginFactory(SpillCPUPluginFactory, {}, {});

SpillCPUPluginFactory::SpillCPUPluginFactory()
    : OFX::PluginFactoryHelper<SpillCPUPluginFactory>(kPluginIdentifier, kPluginVersionMajor, kPluginVersionMinor)
{
}

void SpillCPUPluginFactory::describe(OFX::ImageEffectDescriptor &desc)
{
    // basic labels
...
void SpillCPUPluginFactory::describeInContext(OFX::ImageEffectDescriptor &desc, OFX::ContextEnum /*context*/)
{
...
OFX::ImageEffect* SpillCPUPluginFactory::createInstance(OfxImageEffectHandle handle, OFX::ContextEnum /*context*/)
{
    return new SpillCPUPlugin(handle);
}

void OFX::Plugin::getPluginIDs(PluginFactoryArray& p_FactoryArray)
{
    static SpillCPUPluginFactory inkPlugin;
    p_FactoryArray.push_back(&inkPlugin);
}